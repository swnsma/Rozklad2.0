...
class TeacherModel extends AbstractModel
{
    public function getTeacher()
    {
        ...
        $query = "
SELECT `schedule_flat`.`teacher` as title,
REPLACE(`schedule_flat`.`teacher`,' ','_') as id
FROM `schedule_flat` GROUP by `schedule_flat`.`teacher`";
        $sth = $this->connection->prepare($query);
        $sth->setFetchMode(PDO::FETCH_ASSOC);
        $sth->execute();


        $result['events'] = $this->getLessonsInFormat();

        $result['resources'] = $sth->fetchAll();
    ...
    {
        $query = "
SELECT `schedule_flat`.*,
REPLACE(`schedule_flat`.`teacher`,' ','_') as resourceId
FROM `schedule_flat` ";        $sth = $this->connection->prepare($query);
        $sth->setFetchMode(PDO::FETCH_ASSOC);
        $sth->execute();
        return $sth->fetchAll();
    }
...
//            $record['type'] = $record['type'] == 1 ? 'Практика' : 'Лекція';
    ...
    public function getLessonsInFormat()
    {
        ...
        foreach ($lessons as $lesson) {
            $item = [];
            $item['id'] = rand(5, 15);
            $item['resourceId'] = $lesson['resourceId'];
...
class TeacherController
{
    public function run()
    {
        ...
        echo json_encode($model->getTeacher());
    }
}

$go = new TeacherController();
$go->run();